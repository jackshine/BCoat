 ...
 * @author Eldo Rajan
 */
public class EmailableReporterSauce implements IReporter {
	private static final Logger LOG = Logger.getLogger(EmailableReporterSauce.class);
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		return new PrintWriter(new BufferedWriter(new FileWriter(new File
				(outdir, "emailable-report-sauce.html"))));
	}
	...
	protected void writeSuiteSummary() {
		...
		writer.print("<th>Execution Video Link</th>");
		writer.print("</tr>");

		Map<String,String> urls = getSauceLabsUrl();
		...
		for (SuiteResult suiteResult : suiteResults) {
			...
			for (TestResult testResult : suiteResult.getTestResults()) {
				...
				writeTableData(testResult.getTestName(),urls,false);
				writer.print("</tr>");
	...
	private void writeTableData(String testName, Map<String,String> urls, boolean flag) {
		String url = urls.get(testName);
		writeTableData(url, null);
	}
	...
	private Map<String, String> getSauceLabsUrl() {
		Map<String, String> sauceLabsUrl = new HashMap<String, String>();
		String filePath = System.getProperty("user.dir") +File.separator+"target"+File.separator+"sauceLabsUrl.txt";
		try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
			String line;
			while ((line = br.readLine()) != null) {
				String[] values = line.split("======");
				sauceLabsUrl.put(values[0], values[1]);
			}
		}catch(Exception e){
