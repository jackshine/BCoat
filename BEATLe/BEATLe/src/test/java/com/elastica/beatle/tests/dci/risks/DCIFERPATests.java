...
public class DCIFERPATests extends DCICommonTest implements ITest {
	...
	@Test(dataProvider = "dataUploadFERPAPositive", groups ={"All"})
	public void testDisplayLogsGenerateRiskForFERPAFiles(TestParameters testParams) throws Exception {
	...
	@DataProvider(name = "dataUploadFERPAPositive")
	public Object[][] dataUploadFERPAPositive() {

		dciFunctions = new DCIFunctions();

		
		
		String[] fileName = dciFunctions.getFileName(DCIConstants.DCI_FILE_UPLOAD_FERPA_PATH);	
		String[] risks = dciFunctions.riskTypesForAFile(fileName);
		
			
		fileName = dciFunctions.createSampleFileType(DCIConstants.DCI_FILE_UPLOAD_FERPA_PATH,
				fileName);
		dciFunctions.fileUploadOrSendEmail(suiteData, folderInfo, fileName);

		Object[][] result = new Object[fileName.length][];
		for (int i = 0; i < fileName.length; i++) {
			result[i] = new Object[] { new TestParameters("Risk Generation/Validation for FERPA risks file types:"+fileName[i]+" for risks "+risks, 
					"Upload FERPA risks file:"+fileName[i]+ ". Then verify risk logs are getting generated within the SLA provided"+" for risks "+risks,
	...
	private void verifyDisplayLogs(DCIFunctions dciFunctions, String fileName, String saasType,
			List<NameValuePair> headers, String[] risks, String virusJson) throws Exception {
		String hits = "";
		try {
			...
			String validationMessage = dciFunctions.validationFERPARiskLogs(hits, fileName, suiteData.getUsername(),
