...
public class Audit_SpanVA_Anonymization_Upgrades_Tests extends AuditInitializeTests {
	...
	public void initSpanVAUIConfiguration() throws Exception {
		...
		spanvaFirewalls.load(new FileInputStream(
				FileHandlingUtils.getFileAbsolutePath(AuditTestConstants.SPANVA_UPDRADESTEST_FIREWALLSLIST)));
		
		/*switch (suiteData.getLogCompressionFormat()) {
		case "7z":
			spanvaFirewalls.load(new FileInputStream(
					FileHandlingUtils.getFileAbsolutePath(AuditTestConstants.SPANVA_FIREWALLSLIST_7Z)));
			break;
		case "bz2":
			spanvaFirewalls.load(new FileInputStream(
					FileHandlingUtils.getFileAbsolutePath(AuditTestConstants.SPANVA_FIREWALLSLIST_BZ2)));
			break;
		case "gz":
			spanvaFirewalls.load(new FileInputStream(
		...
		case "zip":
			spanvaFirewalls.load(new FileInputStream(
					FileHandlingUtils.getFileAbsolutePath(AuditTestConstants.SPANVA_FIREWALLSLIST)));
			break;
	...
	@DataProvider(name = "currentVersionProcessedDSS", parallel = true)
	public Object[][] populateCurrentVersionProcessedDatasources() throws Exception {
		
		Reporter.log("scpdatasourcesProp size.."+currentVersionProcessedDS.size(),true);

		Object[][] inputData = new Object[currentVersionProcessedDS.size()][2];
		int j = 0;
		for (String key : currentVersionProcessedDS.stringPropertyNames()) {
			inputData[j][0] = key;
			inputData[j][1] = currentVersionProcessedDS.getProperty(key);
			j++;
		}
		Reporter.log("currentVersionProcessedDS dataprovider.." + inputData, true);

		return inputData;
	}

	@Test(priority=4,dependsOnMethods={"testSpanVAScpDatasourcesVerification"},dataProvider = "currentVersionProcessedDS", threadPoolSize = 7)
	public void testCurrentVersionDatasourcesAnonymization(String fireWallType, String datasourceid) throws Exception {
	
		sourceID=datasourceid;
		Reporter.log("firewall:-"+fireWallType+"sourceID:-"+sourceID,true);
		...
		case AuditTestConstants.FIREWALL_CISCO_ASA_SERIES_GZ:{
			controller = new AuditGoldenSetDataController3(AuditTestConstants.FIREWALL_CISCO_ASA_SERIES_SHEET);
			goldenSetDataList = controller.loadXlData();
			expectedAnonyUsersSet=goldenSetTestDataSetup.getExpectedAnonyUsers(goldenSetDataList);
			actulAnonyUsersSet=AuditTestUtils.getDatasourceAnonyUsers(sourceID,"","","");
			Assert.assertEquals(actulAnonyUsersSet.size(), expectedAnonyUsersSet.size(),"Anony User count is not Matched");
		...
		case AuditTestConstants.FIREWALL_WEBSENSE_HOSTED_GZ:{
			controller = new AuditGoldenSetDataController3(AuditTestConstants.FIREWALL_WEBSENSE_HOSTED_SHEET);
			goldenSetDataList = controller.loadXlData();
			expectedAnonyUsersSet=goldenSetTestDataSetup.getExpectedAnonyUsers(goldenSetDataList);
			actulAnonyUsersSet=AuditTestUtils.getDatasourceAnonyUsers(sourceID,"","","");
			Assert.assertEquals(actulAnonyUsersSet.size(), expectedAnonyUsersSet.size(),"Anony User count is not Matched");
	...
	@Test(priority = 5, dependsOnMethods = "testCurrentVersionDatasourcesAnonymization")
	...
	@DataProvider(name = "upgradesScpVersionProcessedDSS", parallel = true)
	public Object[][] populateUpgradedVersionProcessedDatasources() throws Exception {
		...
		Reporter.log("upgradedVersionProcessedDS dataprovider.." + inputData, true);

		return inputData;
	}

	@Test(priority=7,dependsOnMethods={"testCreateSCPDatasourcesOnUpgradedVersion"},dataProvider = "upgradesScpVersionProcessedDSS", threadPoolSize = 7)
	public void testUpgradedVersionDatasourcesAnonymization(String fireWallType, String datasourceid) throws Exception {
...
//	@DataProvider(name = "test", parallel = true)
	...
	//@Test(priority=8,dependsOnMethods={"testUpgradedVersionDatasourcesAnonymization"},dataProvider = "test", threadPoolSize = 7)
...
/*
	...
	//@Test(priority = 9, dependsOnMethods = "testVerifySpanvaLatestUpdates", dataProvider = "populateHttpsPutDataSources", threadPoolSize = 7)
