...
public class DCIPCITests extends DCICommonTest implements ITest {
	...
	public void testDisplayLogsGenerateRiskForPCIRiskPositiveScenarios(TestParameters testParams) throws Exception {
		...
		verifyDisplayLogs(dciFunctions, testParams.getFileName(), 
				testParams.getSaasType(), headers, risks, testParams.getPrimaryJson());
	...
	@Test(dataProvider = "dataUploadPCINegativeScenarios", groups ={"All"})
	public void testDisplayLogsGenerateRiskForPCIRiskNegativeScenarios(TestParameters testParams) throws Exception {
		dciFunctions = new DCIFunctions();
		esLogs = new ElasticSearchLogs();
	...
	public Object[][] dataUploadPCIPositiveScenarios() {
		...
		String[] fileName = dciFunctions.getFileName(DCIConstants.DCI_FILE_UPLOAD_PCI_POS_PATH);
		String[] risks = dciFunctions.riskTypesForAFile(fileName);

		fileName = dciFunctions.createSampleFileType(DCIConstants.DCI_FILE_UPLOAD_PCI_POS_PATH,
				fileName);

		String[] pciJson = new String[fileName.length];
		...
		for (int i = 0; i < fileName.length; i++) {
			result[i] = new Object[] { new TestParameters("Risk Generation/Validation for PCI risks file:"+fileName[i]+" for risks"+Arrays.asList(risks[i].split(",")), 
					...
					fileName[i], SaasType.getSaasFilterType(suiteData.getSaasApp()), Arrays.asList(risks[i].split(",")), pciJson[i])};
		}
	...
	@DataProvider(name = "dataUploadPCINegativeScenarios")
	public Object[][] dataUploadPCINegativeScenarios() {

		dciFunctions = new DCIFunctions();

		String[] fileName= dciFunctions.getFileName(DCIConstants.DCI_FILE_UPLOAD_PCI_NEG_PATH);
		fileName = dciFunctions.createSampleFileType(DCIConstants.DCI_FILE_UPLOAD_PCI_NEG_PATH,
				fileName);
		dciFunctions.fileUploadOrSendEmail(suiteData, folderInfo, fileName);

		Object[][] result = new Object[fileName.length][];
		for (int i = 0; i < fileName.length; i++) {
			result[i] = new Object[] { new TestParameters("No Risk Generation/Validation for PCI risks file:"+fileName[i], 
