...
public class GoogleDriveRemoveLinkTests extends ProtectInitializeTests {
		...
		while (iterator.hasNext()) {
			try {
				...
				if (policyBean.getExposureType().equalsIgnoreCase("public")) {
					gDrive.insertPermission(gDrive.getDriveService(), fileUploadResponse.getFileId(), null, "anyone", "reader");
				}
				if (policyBean.getExposureType().equalsIgnoreCase("external")) {
					gDrive.insertPermission(gDrive.getDriveService(), fileUploadResponse.getFileId(), "mayurbelekar@gmail.com", "user", "reader");
				}
				if (policyBean.getExposureType().equalsIgnoreCase("internal")) {
					gDrive.insertPermission(gDrive.getDriveService(), fileUploadResponse.getFileId(), "protectbeatle.com", "domain", "reader");
				}
			} catch (Exception e) {
				continue;
			} catch (Error e) {
	 ...
	 * @throws Exception
	 */
	@Test(dataProviderClass = GDriveDataProvider.class, dataProvider = "RemoveLinkDataProvider", priority = 2)
	...
	public Object[][] getDataList() {
		...
		list.add(GDriveDataProvider.getRemoveLinkDataProvider());
