 ...
 *
 */
public class SalesforceContentTypePolicyViolationTests extends ProtectInitializeTests {
	...
	public void testCreatePolicyAndUploadFile(List<String[]> list) throws Exception {
		...
		while (iterator.hasNext()) {
			try {
				...
				File file = protectFunctions.createContentTypeFile(ProtectTestConstants.PROTECT_RESOURCE_CONTENTTYPE_PATH, policyBean.getFileType(), policyBean.getFileExt());
	 ...
	 * @throws Exception
	 */
	@Test(dataProviderClass = SalesforceDataProvider.class, dataProvider = "ContentType", priority = 2)
	public void testPolicyViolationForContentType(String... data) throws Exception {
		PolicyBean policyBean = protectFunctions.populatePolicyBean(data);
		String policyName = policyBean.getPolicyName();
		...
		Reporter.log("Starting testcase: testPolicyViolationForContentType - " + policyName, true);
		...
		Reporter.log("Completed testcase: testPolicyViolationForContentType - " + policyName, true);
	}
	...
	public void testDeletePolicy(List<String[]> list) throws Exception {
		...
		while (iterator.hasNext()) {
			try {
				...
				protectFunctions.deleteCIQProfiles(restClient, requestHeader, suiteData, policyBean.getCiqProfile());
			} catch (Exception e) {
				continue;
			} catch(Error e) {
				continue;
	...
	public Object[][] getDataList() {
		...
		list.add(SalesforceDataProvider.getSalesforceContentTypePolicyViolationsDataProvider());
