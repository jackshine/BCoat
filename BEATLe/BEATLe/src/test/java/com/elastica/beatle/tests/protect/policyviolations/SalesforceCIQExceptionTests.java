 ...
 *
 */
public class SalesforceCIQExceptionTests extends ProtectInitializeTests {
	...
	public void testCreatePolicyAndUploadFile(List<String[]> list) throws Exception {
		...
		while (iterator.hasNext()) {
			try {
				...
				String ciqExceptionName = data[21].toString();
				String source = data[23].toString();
				List<String> riskList = Arrays.asList(data[22].toString());
				...
				protectFunctions.createContentIQProfile(restClient, requestHeader, suiteData, ciqExceptionName, source, riskList);
				policyBean.setCiqProfileException(ciqExceptionName);
	 ...
	 * @throws Exception
	 */
	@Test(dataProviderClass = SalesforceDataProvider.class, dataProvider = "CIQException", priority = 2)
	...
	public void testDeletePolicy(List<String[]> list) throws Exception {
		...
		while (iterator.hasNext()) {
			try {
				...
				protectFunctions.deleteCIQProfiles(restClient, requestHeader, suiteData, data[21].toString());
			} catch (Exception e) {
				continue;
			} catch(Error e) {
				continue;
	...
	public Object[][] getDataList() {
		...
		list.add(SalesforceDataProvider.getSalesforceContentIQExceptionDataProvider());
