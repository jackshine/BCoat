...
public class OnedriveExternalExposuresTests extends SecurletUtils {
	...
	String collaborator = "admin@securlet365mailbeatle.com";
	private String uniqueFilename = "HIPAA_Test2.txt";
	private String destinationFile = "";
	OneDriveUtils onedriveUtils;

	public OnedriveExternalExposuresTests() throws Exception {
	...
	public void initInstance() throws Exception {
		...
		String externalDomainParts[] = StringUtils.split(suiteData.getSaasAppExternalUser1Name(), "@");
		String externalDomain = StringUtils.substringBefore(externalDomainParts[1], ".");
		Reporter.log("External Domain:"+externalDomain, true);
		...
		saasAppExternalUserAccount = new UserAccount(suiteData.getSaasAppExternalUser1Name(), suiteData.getSaasAppExternalUser1Password(), 
																				suiteData.getSaasAppUserRole(), suiteData.getEnvironmentName(), null, externalDomain);
	...
	public void verifyMetricsTotalBeforeExposureWithUIAndAPIServerCalls() throws Exception {
		...
		ExposureTotals apiCallTotals = getExposuresMetricsTotal(elapp.el_office_365.name(), qparams);
		...
		ExposureTotals uiCallTotals = getUIExposuresMetricsTotal(elapp.el_office_365.name(), qparams);
	...
	public void verifyVennDiagramMetricsAfterExternalExposure() throws Exception {
		...
		ExposureTotals apiCallTotalsBeforeExposure = getOffice365ExposureMetricsWithAPI(false, null);
		...
		ExposureTotals uiCallTotalsBeforeExposure = getOffice365ExposureMetricsWithUI(false, null);
		...
		this.collaborateFileWithExternalUser(collaborator, 1);
		...
		ExposureTotals uiCallTotalsAfterExposure =  getOffice365ExposureMetricsWithUI(false, null);
		...
		ExposureTotals apiCallTotalsAfterExposure = getOffice365ExposureMetricsWithAPI(false, null);
		...
		universalApiExternal.disableSharing(destinationFile);
		
		sleep(CommonConstants.FIVE_MINUTES_SLEEP);
		LogUtils.logStep("6. GET External exposure metrics totals with API server and UI server call");
		
		ExposureTotals apiCallTotals = getOffice365ExposureMetricsWithAPI(false, null);
		...
		ExposureTotals uiCallTotals = getOffice365ExposureMetricsWithUI(false, null);
	...
	public void verifyCollaborateFolderAndChangeExternalCollaboratorRoles(String collaborator, String collaboratorRole) throws Exception {
		...
		docparams.add(new BasicNameValuePair(SecurletsConstants.APP,  "Office 365"));
		...
		apiCallDocs = getExposedDocuments(elapp.el_office_365.name(), docparams);
		...
		SecurletDocument documents = getExposedDocuments(elapp.el_office_365.name(), qparams);
		...
		SecurletDocument uiDocuments = getExposedDocumentsWithUIServerCall("Office 365", foldername);
		...
		*/
		...
		documents = getExposedDocuments(elapp.el_office_365.name(), qparams);
		...
		//UI Server 
		/*
		uiDocuments = getExposedDocumentsWithUIServerCall("Office 365", foldername);
	...
	public void verifyVulnerabilityTypes() throws Exception {
		...
		for (String vl : vulnerabilities) {
			...
			VulnerabilityTypes specificVulnerability = getVulnerabilityTypes("el_office_365", additionalParams);
	...
	public void verifyFileTypes() throws Exception {
		...
		//After external exposure
		ExposureTotals fileTypes = getFileTypes(elapp.el_office_365.name(), hmap);
	...
	public void verifyInternalCollabsOnExternalDocuments() throws Exception {		
			...
			SecurletDocument documents = getCollaborators(elapp.el_office_365.name(), docparams);
	...
	public ExposureTotals  getOffice365ExposureMetricsWithAPI(boolean isInternal, List<NameValuePair> queryParams) throws Exception  {
		List<NameValuePair> qparams = new ArrayList<NameValuePair>(); 
	...
	public ExposureTotals  getOffice365ExposureMetricsWithUI(boolean isInternal, List<NameValuePair> queryParams) throws Exception  {
		List<NameValuePair> qparams = new ArrayList<NameValuePair>();
		qparams.clear();
	...
	public void collaborateFileWithExternalUser(String collaborator, int shareAccess) throws Exception {
		String uniqueId = String.valueOf(System.currentTimeMillis());
		destinationFile = uniqueId + "_" + uniqueFilename;
		ItemResource itemResource = universalApiExternal.uploadSimpleFile("/", uniqueFilename, destinationFile);
		SharingUserRoleAssignment shareCollabObject = onedriveUtils.getFileShareObject(itemResource, shareAccess, collaborator);
		universalApiExternal.shareWithCollaborators(shareCollabObject);
