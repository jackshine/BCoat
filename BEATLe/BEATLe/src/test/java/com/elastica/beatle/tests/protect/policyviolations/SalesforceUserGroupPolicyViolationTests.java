 ...
 *
 */
public class SalesforceUserGroupPolicyViolationTests extends ProtectInitializeTests {
	...
	public void init() throws Exception {
		...
		String result = sfapi.executeQuery(query);
	...
	public void testCreatePolicyAndUploadFile(List<String[]> list) throws Exception {
		...
		Map<String, String> groupIds = protectFunctions.getSalesforceGroupIds();
		while (iterator.hasNext()) {
			try {
				...
				if(policyBean.getPolicyType().contains("internalGroup")){
					groupDetails = sfapi.getGroup(groupIds.get("Internal Group"));
					InternalFileShare ifs = new InternalFileShare("V", chatterfile.getId(), groupIds.get("Internal Group"));
					sfapi.shareFileInternally(ifs);
				}
				if(policyBean.getPolicyType().contains("internalGroup1")){
					groupDetails = sfapi.getGroup(groupIds.get("Internal Group 1"));
					InternalFileShare ifs = new InternalFileShare("V", chatterfile.getId(), groupIds.get("Internal Group 1"));
					sfapi.shareFileInternally(ifs);
				}
				if(policyBean.getPolicyType().contains("internalGroup2")){
					groupDetails = sfapi.getGroup(groupIds.get("Internal Group 2"));
					InternalFileShare ifs = new InternalFileShare("V", chatterfile.getId(), groupIds.get("Internal Group 2"));
					sfapi.shareFileInternally(ifs);
				}
	 ...
	 * @throws Exception
	 */
	@Test(dataProviderClass = SalesforceDataProvider.class, dataProvider = "UserGroup", priority = 2)
	...
	public Object[][] getDataList() {
		...
		list.add(SalesforceDataProvider.getSalesforceUserGroupPolicyViolationsDataProvider());
