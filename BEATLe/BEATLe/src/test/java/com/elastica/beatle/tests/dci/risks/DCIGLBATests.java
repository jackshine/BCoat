...
public class DCIGLBATests extends DCICommonTest implements ITest {
	...
	@Test(dataProvider = "dataUploadGLBAPositive", groups ={"All"})
	public void testDisplayLogsGenerateRiskForGLBAFiles(TestParameters testParams) throws Exception {
	...
	@Test(dataProvider = "dataUploadGLBANegative", groups ={"All"})
	public void testDisplayLogsGenerateRiskForGLBARiskNegativeScenarios(TestParameters testParams) throws Exception {
		dciFunctions = new DCIFunctions();
		esLogs = new ElasticSearchLogs();
	...
	@DataProvider(name = "dataUploadGLBAPositive")
	public Object[][] dataUploadGLBAPositive() {

		dciFunctions = new DCIFunctions();


		String[] fileName = dciFunctions.getFileName(DCIConstants.DCI_FILE_UPLOAD_GLBA_PATH);	
		String[] risks = dciFunctions.riskTypesForAFile(fileName);

		fileName = dciFunctions.createSampleFileType(DCIConstants.DCI_FILE_UPLOAD_GLBA_PATH,
					fileName);
		dciFunctions.fileUploadOrSendEmail(suiteData, folderInfo, fileName);

		Object[][] result = new Object[fileName.length][];
		for (int i = 0; i < fileName.length; i++) {
			result[i] = new Object[] { new TestParameters("Risk Generation/Validation for GLBA risks file types:"+fileName[i]+" for risks "+risks[i], 
					"Upload GLBA risks file:"+fileName[i]+ ". Then verify risk logs are getting generated within the SLA provided"+" for risks "+risks[i],
	...
	@DataProvider(name = "dataUploadGLBANegative")
	public Object[][] dataUploadGLBANegative() {

		dciFunctions = new DCIFunctions();

		String[] fileName = dciFunctions.getFileName(DCIConstants.DCI_FILE_UPLOAD_GLBA_NEGATIVE_PATH);	


		fileName = dciFunctions.createSampleFileType(DCIConstants.DCI_FILE_UPLOAD_GLBA_NEGATIVE_PATH,
				fileName);
		dciFunctions.fileUploadOrSendEmail(suiteData, folderInfo, fileName);

		Object[][] result = new Object[fileName.length][];
		for (int i = 0; i < fileName.length; i++) {
			result[i] = new Object[] { new TestParameters("No Risk Generation/Validation for GLBA risks file:"+fileName[i], 
	...
	private void verifyDisplayLogs(DCIFunctions dciFunctions, String fileName, String saasType,
		...
		try {
			...
			String validationMessage = dciFunctions.validationGLBARiskLogs(hits, fileName, suiteData.getUsername(),
