...
public class DCIVirusTests extends DCICommonTest implements ITest {
	...
	@Test(dataProvider = "dataUploadVirusPositive", groups ={"All"})
	public void testDisplayLogsGenerateRiskForVirusFiles(TestParameters testParams) throws Exception {
	...
	@DataProvider(name = "dataUploadVirusPositive")
	public Object[][] dataUploadVirusPositive() {

		dciFunctions = new DCIFunctions();

		String[] fileName = dciFunctions.getFileName(DCIConstants.DCI_FILE_UPLOAD_VIRUS_PATH);	
		String[] risks = dciFunctions.riskTypesForAFile(fileName);
		fileName = dciFunctions.createSampleFileType(DCIConstants.DCI_FILE_UPLOAD_VIRUS_PATH,
					fileName);
		dciFunctions.fileUploadOrSendEmail(suiteData, folderInfo, fileName);

		Object[][] result = new Object[fileName.length][];
		for (int i = 0; i < fileName.length; i++) {
			result[i] = new Object[] { new TestParameters("Risk Generation/Validation for Virus risks file types:"+fileName[i]+" for risks "+Arrays.asList(risks[i].split(",")), 
					"Upload Virus risks file:"+fileName[i]+ ". Then verify risk logs are getting generated within the SLA provided"+" for risks "+Arrays.asList(risks[i].split(",")),
	...
	private void verifyDisplayLogs(DCIFunctions dciFunctions, String fileName, String saasType,
		...
		try {
			...
			String validationMessage = dciFunctions.validationVirusRiskLogs(hits, fileName, suiteData.getUsername(),
