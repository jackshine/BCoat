...
public class GoogleDrivePreventWritersCanShareTests extends ProtectInitializeTests {
	...
	public void init() throws Exception {
		...
		externalUser = ProtectInitializeTests.getRegressionSpecificSuitParameters("externalgdriveUser");
	}
	 ...
	 * @throws Exception
	 */
	@Test(dataProviderClass = GDriveDataProvider.class, dataProvider = "PreventWritersCanShareDataProvider", priority = 2)
	public void testPolicyViolationAndRemediationLogs(String... data) throws Exception {
		...
		universalApi = protectFunctions.loginToGDrive(clientId, clientSecret, refreshtoken, externalUser, "******", "USER");
		gDrive = universalApi.getgDrive();
		String permissionResponse;
		try{
			Permission permission =  gDrive.insertPermission(gDrive.getDriveService(), fileIdCollection.get(policyName), externalUser, "user", policyBean.getPolicyType());
	...
	public Object[][] getDataList() {
		...
		list.add(GDriveDataProvider.getPreventWritersCanShareDataProvider());
