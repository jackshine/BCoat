...
public class GmailActivityLogsTests extends GmailUtils {
	...
	long maxWaitTime = 25;
	long minWaitTime = 10;
	long shortWaitTime = 3;
    String sender=GmailDataProvider.TESTUSER1;
    String sender1=GmailDataProvider.TESTUSER2;
	String clientId;
	String clientSecret;
	String refreshToken;
	...
	public GmailActivityLogsTests() throws Exception {
	...
	@Test(priority= -10, groups={"SEND","GMAIL", "SANITY","REGRESSSION"}, dataProvider = "sendPlainMailDataProvider", dataProviderClass = GmailDataProvider.class)
	public void sendMail(String key, String testDesc, ArrayList<String> to, ArrayList<String> cc, ArrayList<String> bcc, ArrayList<String> inFolder, ArrayList<String> internalRecipients, ArrayList<String> externalRecipients,String email) throws Exception {
	...
	public void fetchActivityLogs() throws Exception {
		...
		try {
			...
			for (int i = 0; i <= (maxWaitTime * 60 * 1000); i+=60000 ) {
				...
				this.sentMessageLogs = this.getInvestigateLogs(-60, 60, facility.Gmail.name(), termmap, suiteData.getUsername(), suiteData.getApiserverHostName(), 
				...
				this.receiveMessageLogs = this.getInvestigateLogs(-60, 60, facility.Gmail.name(), termmap, suiteData.getUsername(), suiteData.getApiserverHostName(), 
						suiteData.getCSRFToken(), suiteData.getSessionID(), 0, 200, "Google Apps");
				...
				this.createMessageLogs = this.getInvestigateLogs(-60, 60, facility.Gmail.name(), termmap, suiteData.getUsername(), suiteData.getApiserverHostName(), 
						suiteData.getCSRFToken(), suiteData.getSessionID(), 0, 200, "Google Apps");
				...
				this.deleteMessageLogs = this.getInvestigateLogs(-60, 60, facility.Gmail.name(), termmap, suiteData.getUsername(), suiteData.getApiserverHostName(), 
						suiteData.getCSRFToken(), suiteData.getSessionID(), 0, 200, "Google Apps");
				...
				this.trashMessageLogs = this.getInvestigateLogs(-60, 60, facility.Gmail.name(), termmap, suiteData.getUsername(), suiteData.getApiserverHostName(), 
						suiteData.getCSRFToken(), suiteData.getSessionID(), 0, 200, "Google Apps");
				...
				this.contentInspectionLogs = this.getInvestigateLogs(-60, 60, facility.Gmail.name(), termmap, suiteData.getUsername(), suiteData.getApiserverHostName(), 
						suiteData.getCSRFToken(), suiteData.getSessionID(), 0, 200, "Google Apps");
	...
	@Test(groups={"SEND", "RECEIVE","CREATE","DELETE","TRASH", "GMAIL", "SANITY"})
	public void fetchSentMessageActivityLogs() throws Exception {
		//Fetch the logs

		try {
			sleep(CommonConstants.FIVE_MINUTES_SLEEP);
			for (int i = 0; i <= (shortWaitTime * 60 * 1000); i+=60000 ) {
				sleep(CommonConstants.SIXTY_SECONDS_SLEEP);
				System.out.println("waiting for fetch logs");
				//sleep(CommonConstants.TEN_MINUTES_SLEEP);
...
//						suiteData.getCSRFToken(), suiteData.getSessionID(), 0, 200, "Google Apps");
	...
	@Test(dependsOnMethods={"fetchActivityLogs"}, dataProvider = "sendPlainMailKeyDataProvider", groups={"SEND", "GMAIL", "SANITY", "REGRESSSION"}, dataProviderClass = GmailDataProvider.class)
	public void verifySendPlainMailActivity(String key) throws Exception {
		LogUtils.logTestDescription("After sending the plain message, check the activity logs.");
		
		ArrayList<String> availableMessages = retrieveActualMessages(sentMessageLogs);
		displayMessageContent(availableMessages);
		//Reporter.log("Expected Message: "+gmailActivities.get(key));
	...
	@Test( dataProvider = "sendMailWithAttachmentDataProviderSanity", groups={"SEND", "GMAIL", "SANITY"}, dataProviderClass = GmailDataProvider.class)
	public void verifySendAttachmentMessageActivitySanity(String key, String testDesc, ArrayList<String> to, ArrayList<String> cc, ArrayList<String> bcc, ArrayList<String> inFolder, ArrayList<String> internalRecipients, ArrayList<String> externalRecipients,String filename,String email) throws Exception {
		...
		Reporter.log("2)verify the sent logs", true);
		Reporter.log("*****************************************************", true);
		
		//=======================================
		Reporter.log("1). Perform Send Mail action", true);
		LogUtils.logTestDescription(testDesc);
		...
		Reporter.log("1).  Send Mail action completed===", true);
				
		//=============================================================
		try {
			sleep(CommonConstants.TEN_MINUTES_SLEEP);
			Reporter.log("Fetching  sent logs", true);
			this.fetchSentMessageActivityLogs();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		LogUtils.logTestDescription("After sending the  message with attachment, verify the sent message activity in the activity logs.");
		Reporter.log("Validating sent logs", true);
	...
	public void verifyRecievePlainMailActivity(String key) throws Exception {
		...
		ArrayList<String> receivedLogs = retrieveActualMessages(receiveMessageLogs);
		displayMessageContent(receivedLogs);
		Reporter.log("Received Log message which shouldn't appear:"+gmailActivities.get(key));
//		logValidator.verifyGmailActivityLog(receiveMessageLogs, gmailActivities.get(key));
		assertFalse(receivedLogs.contains(gmailActivities.get(key)), "Received logs shouldn't be returned for internal mails. Found the log:"+gmailActivities.get(key));
	}
	...
	public void verifyRecieveMailWithAttachmentActivity(String key) throws Exception {
		...
		ArrayList<String> availableMessages = retrieveActualMessages(receiveMessageLogs);
		displayMessageContent(availableMessages);
		//Reporter.log("Expected Message: "+gmailActivities.get(key));
		Reporter.log("Received Log message which shouldn't appear:"+gmailActivities.get(key));
		
		assertFalse(availableMessages.contains(gmailActivities.get(key)), "Received logs shouldn't be returned for internal mails. Found the log:"+gmailActivities.get(key));
		
//		logValidator.verifyGmailActivityLog(receiveMessageLogs, gmailActivities.get(key));
//		assertEquals(Integer.parseInt(receiveMessageLogs.getHits().getTotal().toString()), 0, "Received logs shouldn't be returned for internal mails");
	...
	public void verifyDeletMessageActivity() throws Exception {
		...
		Reporter.log("All messages:"+MarshallingUtils.marshall(deleteMessageLogs), true);
		
		ArrayList<String> availableMessages = retrieveActualMessages(deleteMessageLogs);
		displayMessageContent(availableMessages);
		Reporter.log("ExpectedMessage:"+gmailActivities.get("deleteMessageActivity"));
	...
	public void verifyTrashMessageActivity() throws Exception {
		...
		ArrayList<String> availableMessages = retrieveActualMessages(trashMessageLogs);
		displayMessageContent(availableMessages);
		//Reporter.log("Expected Message: "+gmailActivities.get("trashMailActivity"));
...
//	@Test(groups={"CREATE", "GMAIL", "SANITY", "REGRESSSION"}, dependsOnMethods={"fetchActivityLogs"})
	...
	public void verifySendDraftMessageActivity(String key) throws Exception {
		...
		ArrayList<String> availableMessages = retrieveActualMessages(createMessageLogs);
		displayMessageContent(availableMessages);
		//Reporter.log("Expected Message: "+gmailActivities.get(key));
...
//	@Test(groups={"DELETE","CREATE", "GMAIL", "SANITY", "REGRESSSION"}, dependsOnMethods={"fetchActivityLogs"})
	public void verifyDeleteDraftMessageActivity() throws Exception {
		...
		ArrayList<String> availableMessages = retrieveActualMessages(createMessageLogs);
		displayMessageContent(availableMessages);
		//Reporter.log("Expected Message: "+gmailActivities.get("deleteDraft1"));
	...
	public void verifySendMailWithRisksInline(String key) throws Exception {
		...
		ArrayList<String> availableMessages = retrieveActualMessages(contentInspectionLogs);
		displayMessageContent(availableMessages);
		//Reporter.log("Expected Message: "+gmailActivities.get(key));
	...
	public void verifySendPlainMessageActivity() {
		...
		ArrayList<String> availableMessages = retrieveActualMessages(sentMessageLogs);
		displayMessageContent(availableMessages);
		//Reporter.log("Expected Message: "+gmailActivities.get("sendPlainMessageActivity"));
