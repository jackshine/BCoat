...
public class DCIVBAMacrosTests extends DCICommonTest implements ITest {
	...
	@Test(dataProvider = "dataUploadVBAMacrosPositive", groups ={"All"})
	public void testDisplayLogsGenerateRiskForVBAMacrosFiles(TestParameters testParams) throws Exception {
	...
	@Test(dataProvider = "dataUploadVBAMacrosNegative", groups ={"All"})
	public void testDisplayLogsGenerateRiskForVBAMacrosRiskNegativeScenarios(TestParameters testParams) throws Exception {
		dciFunctions = new DCIFunctions();
		esLogs = new ElasticSearchLogs();
	...
	@DataProvider(name = "dataUploadVBAMacrosPositive")
	public Object[][] dataUploadVBAMacrosPositive() {

		dciFunctions = new DCIFunctions();


		String[] fileName = dciFunctions.getFileName(DCIConstants.DCI_FILE_UPLOAD_VBA_PATH);	
		String[] risks = dciFunctions.riskTypesForAFile(fileName);
		fileName = dciFunctions.createSampleFileType(DCIConstants.DCI_FILE_UPLOAD_VBA_PATH,
				fileName);
		dciFunctions.fileUploadOrSendEmail(suiteData, folderInfo, fileName);

		Object[][] result = new Object[fileName.length][];
		for (int i = 0; i < fileName.length; i++) {
			result[i] = new Object[] { new TestParameters("Risk Generation/Validation for VBA Macros risks file types:"+fileName[i]+" for risks "+risks, 
					"Upload VBA Macros risks file:"+fileName[i]+ ". Then verify risk logs are getting generated within the SLA provided"+" for risks "+risks,
	...
	@DataProvider(name = "dataUploadVBAMacrosNegative")
	public Object[][] dataUploadVBAMacrosNegative() {

		dciFunctions = new DCIFunctions();

		String[] fileName = dciFunctions.getFileName(DCIConstants.DCI_FILE_UPLOAD_VBA_NEGATIVE_PATH);	
		fileName = dciFunctions.createSampleFileType(DCIConstants.DCI_FILE_UPLOAD_VBA_NEGATIVE_PATH,
				fileName);
		dciFunctions.fileUploadOrSendEmail(suiteData, folderInfo, fileName);

		Object[][] result = new Object[fileName.length][];
		for (int i = 0; i < fileName.length; i++) {
			result[i] = new Object[] { new TestParameters("No Risk Generation/Validation for VBA Macros risks file:"+fileName[i], 
	...
	private void verifyDisplayLogs(DCIFunctions dciFunctions, String fileName, String saasType,
		...
		try {
			...
			String validationMessage = dciFunctions.validationVBAMacrosRiskLogs(hits, fileName, suiteData.getUsername(),
