...
public class AuditSpanVAScpClientTests extends AuditInitializeTests {
	...
	public AuditSpanVAScpClientTests(String FireWallName) {
	...
	public void populateSpanvaScpClientData() throws Exception {
		...
		spanvaAwsNodeConnectPEMLocationPath = spanvaconfigProps.getProperty("spanva_nfs_server_cert_pem_location"); //pem file location path to connect to scpclient sever
		firewallLogFilePath = AuditTestUtils.getFirewallLogFilePath(fireWallType); //get the upload firewall path
		spanvaScpClientUsername=spanvaconfigProps.getProperty("spanva_scp_client_username"); //get the spanva scpclient username
		spanVaScpClientUserPwd=spanvaconfigProps.getProperty("spanva_scp_client_userpwd"); //get the spanva scpclient userpwd
		spanVaScpClientHost=spanvaconfigProps.getProperty("spanva_nfs_server_host"); //get the spanva scpclient host
		spanVaScpClientHostSourceDir=getScpClientSourceDirPath(fireWallType); //get the firewall source directory path of scpclient
		...
		spanVAPayload = AuditTestUtils.createSCPClientSUploadPayload
				(fireWallType,agentId,suiteData.getEnvironmentName(),AuditTestConstants.AUDIT_SPANVA_DS_NAME+"SCP_CLIENT",
						spanVaScpClientHostSourceDir,spanVaScpClientHost, spanvaScpClientUsername, spanVaScpClientUserPwd);
	...
	@Test(priority=1)
	public void testSpanvaScpClientDataSourceCreationAndProcess() throws Exception
	{
		...
		SFTPUtils sftpUtils=getSftpUtilsConfiguration(spanvaScpClientUsername,spanVaScpClientUserPwd,spanVaScpClientHost,spanVaScpClientHostSourceDir);
		//SFTPUtils sftpUtils = getSftpUtilsConfiguration("ubuntu", spanvaNfsServerHost, spanVaScpClientHostSourceDir);
	...
	@Test(priority=2,dependsOnMethods={"testSpanvaScpClientDataSourceCreationAndProcess"})
...
/*
	...
	@Test(priority=3,dependsOnMethods={"testSpanvaScpClientDataSourceCreationAndProcess"})
