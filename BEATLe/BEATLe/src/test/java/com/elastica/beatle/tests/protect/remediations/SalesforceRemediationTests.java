 ...
 *
 */
public class SalesforceRemediationTests extends ProtectInitializeTests {
	...
	public void testCreatePolicyAndUploadFile(List<String[]> list) throws Exception {
		...
		while (iterator.hasNext()) {
			try {
				...
				if(policyBean.getPolicyType().equals("shareFileInternallyView")){
					Logger.info("Share the file publicly.");
					sfapi.createFileShareLink(chatterfile.getId());
				...
				if(policyBean.getPolicyType().equals("shareFileInternallyCollab")){
					Logger.info("Share the file publicly.");
					sfapi.createFileShareLink(chatterfile.getId());
				...
				if(policyBean.getPolicyType().equals("shareFileInternalUserView")){
					String userId1 = sfapi.getUserId(internalUserId, "Securlet User 1", "Int1", "SecBeatle1");
					String ids[] = {userId1};
				...
				if(policyBean.getPolicyType().equals("shareFileInternalUserCollab")){
					String userId1 = sfapi.getUserId(internalUserId, "Securlet User 1", "Int1", "SecBeatle1");
					String ids[] = {userId1};
				...
				if(policyBean.getPolicyType().equals("shareFileExternallyView")){
				...
				if(policyBean.getPolicyType().equals("shareFileExternallyEdit")){
					String userId = sfapi.getExternalUserId(externalUserId1, "Protect Auto 1", "SUser", "ExtUser11");
					String ids[] = {userId};
					String sharingTypes[] = {"C"};
					FileSharesInput fsi = securletUtils.frameFileShareInputObject(ids, sharingTypes, "I have shared a file with edit permission.");
	 ...
	 * @throws Exception
	 */
	@Test(dataProviderClass = SalesforceDataProvider.class, dataProvider = "Remediations", priority = 2)
	...
	public Object[][] getDataList() {
		...
		list.add(SalesforceDataProvider.getSalesforceRemediationDataProvider());
